...
object Anagrams extends AnagramsInterface :
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(e => e).transform((_, s) => s.length).toList.sortBy((c, _) => c)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(Occurrences.empty)
    case (char, times) :: tail =>
      for {
        combs <- combinations(tail)
        n <- 0 to times
      } yield if (n == 0) combs else (char, n) :: combs
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = for {
    xs <- x
    subs = y.toMap.get(xs._1).map(i => xs._2 - i)
    if subs.isEmpty || subs.get > 0
  } yield if (subs.isEmpty) xs else (xs._1, subs.get)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def _internal(occ: Occurrences): List[Sentence] = {
      if (occ.isEmpty) List(Sentence.empty)
      else {
        for {
          combs <- combinations(occ)
          if dictionaryByOccurrences.contains(combs)
          words <- dictionaryByOccurrences(combs)
          rest <- _internal(subtract(occ, combs))
        } yield words :: rest
      }
    }

    _internal(sentenceOccurrences(sentence))
  }
