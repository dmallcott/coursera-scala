...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet :
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val nextSet = if p(elem) then acc.incl(elem) else acc
    left.filterAcc(p, right.filterAcc(p, nextSet))
  }
  ...
  def mostRetweetedAcc(top: Tweet): Tweet =
    if elem.retweets > top.retweets then left.mostRetweetedAcc(right.mostRetweetedAcc(elem))
    else left.mostRetweetedAcc(right.mostRetweetedAcc(top))
